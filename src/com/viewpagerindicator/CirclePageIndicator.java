...
public class CirclePageIndicator extends View implements PageIndicator {
	 ...
	 * 是否启用动画（像青蛙跳跳）
	 */
	private boolean animEabled = true;
	...
	public boolean isAnimEabled() {
		return animEabled;
	}

	public void setAnimEabled(boolean animEabled) {
		this.animEabled = animEabled;
		this.mSnap = !animEabled;
	}
	...
	private int getAdapterCount() {
		...
		if (isRepeat) {
			adapterCount = repeatCount;
		} else
			adapterCount = mViewPager.getAdapter().getCount();
	...
	protected void onDraw(Canvas canvas) {
		...
		if (isRepeat) {
			mCurrentPage = mCurrentPage % repeatCount;
		}
		...
		// LogUtils.printLog("mCurrentPage:" + mCurrentPage + " mPageOffset:"
		// + mPageOffset);
		...
		if (!mSnap) {
			...
			if (isRepeat && mCurrentPage == repeatCount - 1 && mPageOffset > 0) {
				cx -= mPageOffset * (repeatCount - 1) * threeRadius;
			} else
		...
		// System.out.println("mSnapPage:" + mSnapPage + " mCurrentPage:"
		// + mCurrentPage + " mSnap:" + mSnap + " mPageOffset:"
		// + mPageOffset + " dX:" + dX + " longOffset:" + longOffset
		// + " shortOffset:" + shortOffset + " cx:" + cx);
	...
	public boolean onTouchEvent(android.view.MotionEvent ev) {
		...
		if ((mViewPager == null) || (getAdapterCount() == 0)) {
			return false;
		}
	 ...
	 * 
	 * @Title setViewPager
	 * @Description 设置重复数目和CirclePageIndicator动画启用
	 * @param view
	 *            ViewPager
	 * @param repeatCount
	 *            重复几个？
	 * @param animEabled
	 *            是否启用CirclePageIndicator青蛙跳跳动画
	 * 
	 * @return void 返回类型
	 */
	public void setViewPager(ViewPager view, int repeatCount, boolean animEabled) {

		this.repeatCount = repeatCount;
		this.isRepeat = (repeatCount > 0 ? true : false);
		this.animEabled = animEabled;
		this.mSnap = !animEabled;
	 ...
	 * @return void 返回类型
	 */
	public void setViewPager(ViewPager view, int repeatCount,
			int initialPosition, boolean animEabled) {

		setViewPager(view, repeatCount, animEabled);
	...
	public void onPageScrolled(int position, float positionOffset,
		...
		if (isRepeat) {
			position = position % repeatCount;
			positionOffset = positionOffset % repeatCount;
		}
	...
	public void onRestoreInstanceState(Parcelable state) {
		...
		if (isRepeat) {
			savedState.currentPage = savedState.currentPage % repeatCount;
		}
	...
	public Parcelable onSaveInstanceState() {
		...
		SavedState savedState = new SavedState(superState, repeatCount,
				isRepeat);
	...
	static class SavedState extends BaseSavedState {
		...
		public SavedState(Parcelable superState, int repeatCount,
				boolean isRepeat) {
			super(superState);
		}
		...
		private SavedState(Parcel in) {
			...
			if (isRepeat) {
				currentPage = in.readInt() % repeatCount;
			} else
