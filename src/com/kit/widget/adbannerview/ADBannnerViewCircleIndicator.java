package com.kit.widget.adbannerview;
 ...
 * </ul>
 */
public class ADBannnerViewCircleIndicator extends View implements Indicator,
		AnimationListener {
	...
	private boolean isThisCircleEnd = true;
	 ...
	 * @param context
	 */
	public ADBannnerViewCircleIndicator(Context context) {
		super(context);
	 ...
	 * @param attrs
	 */
	public ADBannnerViewCircleIndicator(Context context, AttributeSet attrs) {
		super(context, attrs);
		// Retrieve styles attributs
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.ADBannnerViewCircleIndicator);

		// Gets the inactive circle type, defaulting to "fill"
		int activeType = a
				.getInt(R.styleable.ADBannnerViewCircleIndicator_ADBannnerViewCircleIndicator_activeType,
						STYLE_FILL);
		...
		int activeColor = a
				.getColor(
						R.styleable.ADBannnerViewCircleIndicator_ADBannnerViewCircleIndicator_activeColor,
						activeDefaultColor);

		// Gets the inactive circle type, defaulting to "stroke"
		int inactiveType = a
				.getInt(R.styleable.ADBannnerViewCircleIndicator_ADBannnerViewCircleIndicator_inactiveType,
						STYLE_STROKE);
		...
		int inactiveColor = a
				.getColor(
						R.styleable.ADBannnerViewCircleIndicator_ADBannnerViewCircleIndicator_inactiveColor,
						inactiveDefaultColor);

		useAnimation = a
				.getBoolean(
						R.styleable.ADBannnerViewCircleIndicator_ADBannnerViewCircleIndicator_useAnimation,
						false);

		// Retrieve the radius
		radius = a
				.getDimension(
						R.styleable.ADBannnerViewCircleIndicator_ADBannnerViewCircleIndicator_radius,
						4.0f);

		circleSeparation = a
				.getDimension(
						R.styleable.ADBannnerViewCircleIndicator_ADBannnerViewCircleIndicator_circleSeparation,
						2 * radius + radius);
		activeRadius = a
				.getDimension(
						R.styleable.ADBannnerViewCircleIndicator_ADBannnerViewCircleIndicator_activeRadius,
						0.5f);
		// Retrieve the fade out time
		// fadeOutTime = a
		// .getInt(R.styleable.ADBannnerViewCircleIndicator_ADBannnerViewCircleIndicator_fadeOut,
		// 0);

		mCentered = a
				.getBoolean(
						R.styleable.ADBannnerViewCircleIndicator_ADBannnerViewCircleIndicator_centered,
						false);
	...
	protected void onDraw(Canvas c) {
		...
		if (adBannerView.getCurrIndicatorNum() == count - 1) {
			lastCx = cx;
		}

//		System.out.println("adBannerView.getCurrIndicatorNum():"
//				+ adBannerView.getCurrIndicatorNum());

		if (!useAnimation) {
			canvas.drawCircle(
					leftPadding
							+ radius
							+ (adBannerView.getCurrIndicatorNum() * circleSeparation)
							+ centeringOffset, getPaddingTop() + radius, radius
							+ activeRadius, mPaintActive);

		} else {

			if (adBannerView.getCurrIndicatorNum() == 0) {
	 ...
	 */
	@Override
	public void setADBannerView(ADBannerView view) {
		// resetTimer();
		adBannerView = view;
		flowWidth = adBannerView.getWidth();
		invalidate();
	}
	...
	public void onScrolled(int h, int v, int oldh, int oldv) {
		...
		// resetTimer();
		flowWidth = adBannerView.getWidth();
		if (adBannerView.getViewsCount() * flowWidth != 0) {
			currentScroll = h % (adBannerView.getViewsCount() * flowWidth);
		} else {
			currentScroll = h;
		}
		invalidate();
